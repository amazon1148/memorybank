...
import { toTildePath, getDocsPathValue, validateRepositories } from "./memorybank-status.js"
...
describe("memorybank-status", () => {
	...
	describe("validateRepositories", () => {
		...
		it("should throw error when file is missing", async () => {
			...
			await expect(validateRepositories(testDir)).rejects.toThrow(`Required file not found: ${requiredFiles[0]}`)
		})
	})
})
