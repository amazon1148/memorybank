...
async function getFileGitStatus(
	...
	gitPath: string,
	cwd: string,
): Promise<{ icon: string; status: string }> {
...
async function getRepositoryInfo(docsPath: string): Promise<void> {
	...
	const files = ["productContext.md", "activeContext.md", "systemPatterns.md", "techContext.md", "progress.md"]
	...
	try {
		...
		for (const file of files) {
			const filePath = path.resolve(docsPath, file)
			const status =
				error instanceof Error && "code" in error && error.code === 127 ? "git not available" : "not in git"
 ...
 * @param showIncomplete Whether to only show incomplete items
 */
function printSubsection(subsection: MemorybankSubsection, showIncomplete: boolean): void {
